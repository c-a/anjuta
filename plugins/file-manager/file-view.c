...
file_view_select_from_iter (AnjutaFileView* view, GtkTreeIter iter)
{
	...
	do
	{
		...
		if (g_file_equal (priv->selected_file, file))
		{
			file_view_select_iter (view, iter);
			break;
		}

		else if (g_file_has_prefix (priv->selected_file, file))
		{
			if (is_dir)
			{
							  ...
							  gpointer user_data)
{
	...
	if (priv->selected_file)
	{
		...
		if (g_file_has_prefix (priv->selected_file, dir))
...
file_view_set_selected (AnjutaFileView* view, GFile *selected)
{
	...
	priv->selected_file = g_object_ref (selected);
...
file_view_init (AnjutaFileView *object)
{
	...
	g_signal_connect (priv->model, "directory-expanded",
					  G_CALLBACK (file_view_directory_expanded), object);
