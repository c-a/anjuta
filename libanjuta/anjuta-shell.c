 ...
 *
 * Load the session represented by @session.
 */
void
anjuta_shell_session_load (AnjutaShell *shell, AnjutaSession *session,
						   GError **error)
{
	g_return_if_fail (ANJUTA_IS_SHELL (shell));

	ANJUTA_SHELL_GET_IFACE (shell)->session_load (shell, session);
}
 ...
 */
void
anjuta_shell_session_save (AnjutaShell *shell, GError **error)
{
	g_return_if_fail (ANJUTA_IS_SHELL (shell));

	ANJUTA_SHELL_GET_IFACE (shell)->session_save (shell);
}

/**
 * anjuta_shell_get_session:
 * @shell: A #AnjutaShell interface
 * 
 * Get the currently loaded session.
 * 
 * Returns: (transfer none); The currently loaded #AnjutaSession.
 */
AnjutaSession *
anjuta_shell_get_session (AnjutaShell *shell)
{
	g_return_val_if_fail (ANJUTA_IS_SHELL (shell), NULL);

	return ANJUTA_SHELL_GET_IFACE (shell)->get_session (shell);
}
